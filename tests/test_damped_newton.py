...
def test_damped_newton_backtracking():
    # kötü bir başlangıç noktasından sabit damping ile kurtulmayı dener
    result = damped_newton(f1, df1, 0, alpha=1.0)
    assert result["method"] == "damped_newton"
    assert result["converged"]
    assert abs(result["root"] - 1.5213797) < 1e-7
    assert isinstance(result["iterations"], int)
...
def test_damped_newton_no_convergence():
    ...
    result = damped_newton(f1, df1, 0, max_iter=10, alpha=1.0)
    assert result["method"] == "damped_newton"
