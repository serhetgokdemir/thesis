...
In this experiment, Newton’s method reached the root in only a small number of iterations and produced very small residual values. This is not surprising, since the method is expected to show quadratic convergence when the starting point is close enough to the root.

The secant method also performed well. It converged quickly and had the advantage of not requiring an explicit derivative calculation. Its convergence was a little slower than Newton’s method, but the method was still efficient in terms of computation.

The damped Newton method gave results close to those of the standard Newton method. In this example, the initial approximation was already near the root, so the damping mechanism did not change the behavior of the method very strongly.

The bisection method required more iterations than the other methods. This is expected because it has linear convergence. However, it steadily narrowed the bracketing interval at each step, which made its behavior stable and reliable during the experiment.

Brent’s method gave one of the most balanced results. It preserved the reliability of a bracketing method, while using interpolation steps to improve the speed of convergence.
