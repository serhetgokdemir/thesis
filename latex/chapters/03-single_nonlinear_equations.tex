...
Brent’s method usually moves toward the root quite fast, but it still keeps the root bracketed at each step of the computation. This is useful in practice because the method does not rely only on speed it also keeps the safety provided by bracketing methods. For this reason, Brent’s method is a common choice in many numerical software libraries.

\subsection{Comparison for the Example}

The numerical results above show that the methods behave quite differently. Bisection is the slowest method in this example, but its progress is regular and easy to follow. The secant method reaches the solution faster and does not require any derivative information. Newton’s method gives the fastest convergence here. The damped Newton method is slower than the standard Newton method, but this loss of speed comes with better stability. Brent’s method gives a balanced result, since it remains reliable while still converging efficiently.
...
The numerical methods for solving single nonlinear equations have been described in the previous sections of this chapter. In this part, a comparison of their properties is done.
...
The secant method is generally more efficient than the bisection method because it uses interpolation information and does not require derivative evaluations. Its convergence is superlinear, with order approximately \(p \approx 1.618\). However, unlike the bisection method, convergence is not guaranteed \cite{kelley}.

The fastest algorithm in the list above is the Newton method. Newton's method has quadratic convergence when the initial approximation is sufficiently close to the root and \(f'(x^*) \neq 0\). This property makes it a very fast method of solving equations. Yet, it can diverge depending on the initial value \cite{burden, kelley}.
